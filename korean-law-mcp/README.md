...
이 서버의 툴로 그대로 재현되는 예다.

```
search_interpretation("개인지방소득세")   → 0건
search_interpretation("지방소득세")       → 2건
```

같은 함정이 법제처 API 전반에 걸쳐 있다. 이 서버가 노출하지 않는 조세심판원 검색(`target=ttSpecialDecc`)에서도 동일하게 관찰됐다 — `"양도소득 개인지방소득세 무신고가산세"`는 0건, `"개인지방소득세"`는 26건이었다. 특정 툴의 버그가 아니라 API의 성질이라는 뜻이다.
