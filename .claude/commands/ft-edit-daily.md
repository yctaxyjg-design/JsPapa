...
- `WebFetch` 호출, URL: `https://www.ft.com/ft-edit`
- 응답 HTML을 임시 파일로 저장 → `bash scripts/ft-edit/parse_index.sh <html>` 호출
...
- 각 서브에이전트는 자기 몫의 URL을 `WebFetch`하고 `parse_article.sh`로 JSON 추출, 결과 리스트 반환
