...
질문을 줄단위 파일(또는 표준입력 `-`)로 주면 JSON 배열로 일괄 반환합니다.

```sh
printf '리스 사용권자산 측정\n이연법인세 일시적차이\n수익 5단계 모형\n' \
  | python build/rag_ollama.py --batch - --retrieve-only --json
# 또는 파일로
python build/rag_ollama.py --batch questions.txt --json
```

### MCP 서버 (로컬 AI가 '도구'로 호출)

`build/mcp_server.py`는 KIFRS RAG를 MCP 도구로 노출합니다(표준 라이브러리만, 의존성 0).
도구 두 개: `kifrs_search`(검색 근거만), `kifrs_answer`(검색+qwen3 답변).

클라이언트(로컬 AI/에이전트)에 등록:
...
모델/호스트는 환경변수로 조정: `OLLAMA_HOST`, `KIFRS_EMBED_MODEL`, `KIFRS_LLM_MODEL`, `KIFRS_CORPUS`.
