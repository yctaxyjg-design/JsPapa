...
EMBEDDED_CORPUS = {
  ...
  "standards": [
    ...
    { "no": "1002", "ifrs": "IAS 2", "title": "재고자산", "category": "자산", "summary": "재고자산의 취득원가 결정, 비용으로 인식하는 방법, 순실현가능가치로의 감액을 규정한다. 매입원가·전환원가(고정제조간접원가 정상조업도 배부)·기타원가, 개별법/선입선출법/가중평균법(후입선출 불허), 저가법(원가와 NRV 중 낮은 금액)과 평가손실·환입을 다룬다.", "keywords": ["재고자산", "취득원가", "매입원가", "전환원가", "고정제조간접원가", "정상조업도", "순실현가능가치", "저가법", "선입선출법", "가중평균법", "개별법", "후입선출 불허", "평가손실", "평가손실환입"], "chunks": [
      "취득원가의 구성: 재고자산 원가는 매입원가(매입가격 + 수입관세·제세금 + 매입부대원가, 매입할인·리베이트 차감), 전환원가(직접노무원가 + 제조간접원가), 그 밖에 현재의 장소·상태에 이르게 하는 데 발생한 기타원가로 구성된다.",
      "전환원가와 제조간접원가 배부: 고정제조간접원가는 생산설비의 정상조업도에 기초해 전환원가에 배부한다. 실제조업도가 정상조업도에 가까우면 실제조업도를 쓸 수 있으나, 낮은 조업도·유휴설비 때문에 배부액을 늘리지 않는다(미배부액은 발생기간 비용).",
      "단위원가 결정방법: 통상 상호 교환될 수 없는 항목이나 특정 프로젝트별 재화·용역은 개별법을 쓴다. 그 외에는 선입선출법(FIFO) 또는 가중평균법을 적용하며, 성격·용도가 유사한 재고에는 동일한 방법을 쓴다. 후입선출법(LIFO)은 허용하지 않는다.",
      "순실현가능가치(NRV)와 저가법: 재고자산은 취득원가와 순실현가능가치 중 낮은 금액으로 측정한다. NRV는 정상적 영업과정의 예상 판매가격에서 완성까지 필요한 추정원가와 판매에 필요한 추정원가를 뺀 금액이다. 보통 항목별로 저가법을 적용한다.",
      "평가손실과 환입: 원가가 NRV를 초과하면 그 차액을 재고자산평가손실로 인식해 장부금액을 NRV로 감액한다. 이후 NRV가 회복되면 최초 취득원가를 한도로 평가손실을 환입(매출원가 등에서 차감)한다.",
      "비용 인식: 재고자산을 판매한 기간에 그 장부금액을 매출원가로 비용 인식하며, 평가손실·감모손실도 발생기간의 비용으로 인식한다."
    ] },
    ...
    { "no": "1008", "ifrs": "IAS 8", "title": "회계정책, 회계추정치 변경과 오류", "category": "재무제표 표시·공시", "summary": "회계정책의 선택과 적용, 회계정책 변경·회계추정치 변경·전기오류 수정의 회계처리와 공시를 규정한다. 회계정책 변경과 오류수정은 소급(재작성), 회계추정치 변경은 전진적용한다.", "keywords": ["회계정책", "회계추정치 변경", "전기오류", "오류수정", "소급적용", "소급재작성", "전진적용", "실무적 불가능"], "chunks": [
      "회계정책의 선택: 거래·사건에 구체적으로 적용되는 기준서가 있으면 그에 따른다. 없으면 경영진의 판단으로 목적적합하고 신뢰성 있는 정보를 제공하는 정책을 개발하되, 유사·관련 기준서와 개념체계의 정의·인식기준을 참고한다.",
      "회계정책의 변경: 기준서에서 요구하거나, 변경이 더 목적적합하고 신뢰성 있는 정보를 제공하는 경우에만 변경한다. 원칙적으로 소급적용하여 비교표시 재무제표를 재작성하고 가장 이른 기간의 이월 이익잉여금 등을 조정한다(실무적으로 불가능하면 가능한 시점부터 전진).",
      "회계추정치의 변경: 자산·부채의 현재 상태나 미래효익·의무에 대한 새로운 정보로 장부금액·소비액을 조정하는 것으로, 전진적용한다(변경한 기간, 또는 그 기간과 미래기간의 손익에 반영). 예: 대손추정, 재고 진부화, 내용연수·잔존가치, 감가상각방법.",
      "정책 변경과 추정 변경의 구분: 측정기준의 변경은 회계정책 변경이다. 측정에 내재된 불확실성을 반영하는 조정은 회계추정치 변경이며, 구분이 어려우면 회계추정치 변경으로 본다.",
      "전기오류의 수정: 중요한 전기오류는 발견한 기간의 재무제표 발행승인 전에 소급재작성한다. 오류가 발생한 기간의 비교금액을 재작성하거나, 그 이전이면 가장 이른 기간의 자산·부채·자본 기초금액을 재작성한다. 당기손익으로 처리하지 않는다."
    ] },
    ...
    { "no": "1016", "ifrs": "IAS 16", "title": "유형자산", "category": "자산", "summary": "유형자산의 인식, 취득원가 측정, 후속측정(원가모형·재평가모형), 감가상각, 제거를 규정한다. 취득원가 구성, 후속원가 자본화, 재평가모형(재평가잉여금·OCI), 감가상각대상금액·내용연수·상각방법, 구성요소별 상각, 손상(제1036호)을 다룬다.", "keywords": ["유형자산", "감가상각", "감가상각대상금액", "원가모형", "재평가모형", "재평가잉여금", "내용연수", "잔존가치", "취득원가", "후속원가", "구성요소별 상각"], "chunks": [
      "인식: 유형자산은 그 자산에서 미래경제적효익이 유입될 가능성이 높고 원가를 신뢰성 있게 측정할 수 있을 때 자산으로 인식하며, 최초에는 원가로 측정한다.",
      "취득원가의 구성: 구입가격(관세·환급불가능 취득세 포함, 매입할인·리베이트 차감), 경영진이 의도한 방식으로 가동하는 데 직접 관련되는 원가(설치·운반·시운전 등), 해체·복구원가의 최초 추정치로 구성된다.",
      "후속원가: 정기적 교체부품 원가나 종합검사(major inspection) 원가는 인식기준을 충족하면 자본화하고 이전 부분의 장부금액은 제거한다. 일상적인 수선·유지 원가는 발생 시 당기비용으로 인식한다.",
      "후속측정 모형 선택: 원가모형(원가-감가상각누계액-손상차손누계액)과 재평가모형 중 하나를 회계정책으로 선택해 유형자산 분류 전체에 일관 적용한다.",
      "재평가모형: 공정가치를 신뢰성 있게 측정할 수 있으면 재평가금액으로 계상한다. 재평가증가액은 기타포괄손익(재평가잉여금)으로, 재평가감소액은 당기손익으로 인식하되 같은 자산의 기존 재평가잉여금이 있으면 그 한도까지 기타포괄손익에서 차감한다.",
      "감가상각: 감가상각대상금액(취득원가-잔존가치)을 내용연수에 걸쳐 체계적으로 배분한다. 방법은 정액법·체감잔액법·생산량비례법 등 효익 소비형태를 반영해 선택하고, 잔존가치·내용연수·상각방법은 매 회계연도 말 재검토한다.",
      "구성요소별 상각: 유형자산의 원가에서 유의적인 부분(예: 항공기 엔진과 동체)은 각각 별도로 구분해 감가상각한다.",
      "제거: 처분하거나 사용·처분으로부터 미래경제적효익이 기대되지 않을 때 제거하며, 처분대가와 장부금액의 차액을 처분손익으로 당기손익에 인식한다. 손상은 제1036호를 적용한다."
    ] },
    ...
    { "no": "1023", "ifrs": "IAS 23", "title": "차입원가", "category": "부채·비용", "summary": "적격자산의 취득·건설·생산과 직접 관련된 차입원가의 자본화를 규정한다. 적격자산 정의, 특정차입금·일반차입금(자본화이자율)의 자본화 대상금액, 자본화 개시·중단·종료 시점을 다룬다.", "keywords": ["차입원가", "자본화", "적격자산", "특정차입금", "일반차입금", "자본화이자율", "일시투자수익", "자본화 개시", "자본화 중단", "이자비용"], "chunks": [
      "자본화 원칙: 적격자산의 취득·건설·생산에 직접 관련되는 차입원가는 그 자산 원가의 일부로 자본화하고, 그 밖의 차입원가는 발생기간의 비용으로 인식한다.",
      "적격자산: 의도된 용도로 사용하거나 판매할 수 있는 상태가 되는 데 상당한 기간이 걸리는 자산이다(예: 제조설비, 건물, 일부 재고). 취득 시점에 사용·판매 가능한 자산이나 단기간 생산 재고는 제외한다.",
      "자본화 대상금액 — 특정차입금: 적격자산 취득 목적으로 특정하여 차입한 자금은 그 기간의 실제 차입원가에서 그 자금의 일시적 투자수익을 차감한 금액을 자본화한다.",
      "자본화 대상금액 — 일반차입금: 일반 목적으로 차입해 적격자산에 사용한 경우, 자산 지출액에 자본화이자율(특정차입금 제외한 일반차입금의 가중평균 차입이자율)을 곱해 자본화하되, 그 기간 실제 발생한 차입원가를 한도로 한다.",
      "자본화 기간: 적격자산 지출·차입원가 발생·취득활동 진행이 모두 시작될 때 자본화를 개시한다. 취득활동을 상당기간 중단하면 자본화를 중단하고, 자산이 의도된 용도로 사용·판매 가능한 상태가 되면(실질적으로 완료되면) 자본화를 종료한다."
    ] },
    ...
    { "no": "1037", "ifrs": "IAS 37", "title": "충당부채, 우발부채, 우발자산", "category": "부채·비용", "summary": "충당부채의 인식·측정과 우발부채·우발자산의 공시를 규정한다. 인식 3요건(현재의무·자원유출 가능성 높음·신뢰성 추정), 의제의무, 최선의 추정치와 현재가치 측정, 우발부채·우발자산의 공시, 미래영업손실, 손실부담계약, 구조조정충당부채를 다룬다.", "keywords": ["충당부채", "우발부채", "우발자산", "현재의무", "의제의무", "자원유출 가능성", "최선의 추정치", "현재가치", "손실부담계약", "구조조정충당부채", "미래영업손실"], "chunks": [
      "충당부채의 인식: ① 과거사건의 결과로 현재의무(법적의무 또는 의제의무)가 존재하고, ② 그 의무를 이행하기 위해 경제적효익을 갖는 자원의 유출 가능성이 높으며, ③ 의무 이행에 필요한 금액을 신뢰성 있게 추정할 수 있을 때 충당부채를 인식한다. 세 요건을 모두 충족해야 한다.",
      "현재의무와 의제의무: 의무발생사건은 법적의무를 생기게 하거나, 기업이 책임을 부담할 것이라는 정당한 기대를 상대방에게 갖게 하여 의제의무를 생기게 하는 과거사건이다.",
      "측정: 보고기간말 현재 의무 이행에 필요한 지출에 대한 최선의 추정치로 측정한다. 다수의 항목이면 기댓값, 단일 의무이면 가장 가능성이 높은 결과를 사용한다. 화폐의 시간가치가 유의적이면 현재가치로 할인하고, 위험과 불확실성을 반영한다.",
      "우발부채: 잠재적 의무이거나 인식요건을 충족하지 못하는 현재의무는 충당부채로 인식하지 않고 주석으로 공시한다. 자원유출 가능성이 희박하면 공시도 생략한다.",
      "우발자산: 인식하지 않으며, 경제적효익의 유입 가능성이 높으면 공시한다. 유입이 거의 확실해지면 그 자산을 인식한다.",
      "충당부채의 사용·변동: 충당부채는 최초 인식 대상이 된 지출에만 사용하고, 보고기간말마다 재검토해 더 이상 자원유출 가능성이 높지 않으면 환입한다.",
      "미래영업손실: 미래의 예상 영업손실에 대해서는 충당부채를 인식하지 않는다.",
      "손실부담계약: 계약상 의무 이행에 필요한 회피불가능한 원가가 그 계약에서 받을 것으로 기대되는 경제적효익을 초과하는 손실부담계약은 현재의무를 충당부채로 인식한다.",
      "구조조정충당부채: 구조조정의 공식적이고 구체적인 계획이 있고, 그 계획의 이행에 착수했거나 주요 내용을 공표해 영향받는 자에게 정당한 기대를 갖게 했을 때 인식한다. 구조조정과 직접 관련되고 불가피한 지출만 포함하며, 계속 활동 관련 원가는 제외한다."
    ] },
    { "no": "1038", "ifrs": "IAS 38", "title": "무형자산", "category": "자산", "summary": "무형자산의 인식, 취득원가 측정, 후속측정, 상각을 규정한다. 식별가능성·통제·미래효익, 개별취득·사업결합취득·내부창출, 연구단계 비용화와 개발단계 자본화 요건, 원가·재평가모형, 유한·비한정 내용연수와 상각·손상을 다룬다.", "keywords": ["무형자산", "식별가능성", "통제", "연구단계", "개발단계", "개발비", "내부창출", "상각", "유한 내용연수", "비한정 내용연수", "재평가모형", "영업권 제외"], "chunks": [
      "정의: 무형자산은 물리적 실체가 없는 식별가능한 비화폐성 자산이다. 식별가능성은 자산이 분리가능하거나(매각·이전·라이선스 가능), 계약상 권리 또는 기타 법적 권리에서 발생할 때 충족된다.",
      "인식과 측정: 자산에서 미래경제적효익이 유입될 가능성이 높고 원가를 신뢰성 있게 측정할 수 있을 때 인식하며, 기업이 그 효익을 통제할 수 있어야 한다. 최초에는 원가로 측정한다.",
      "취득 형태: 개별취득(원가), 사업결합으로 취득(취득일의 공정가치), 내부창출로 구분한다. 사업결합으로 취득한 무형자산은 영업권과 분리해 인식할 수 있다.",
      "내부창출 — 연구·개발: 연구단계 지출은 전액 발생 시 비용으로 인식한다. 개발단계 지출은 기술적 실현가능성, 완성·사용·판매 의도, 사용·판매 능력, 미래경제적효익 창출방법, 충분한 자원, 지출의 신뢰성 있는 측정의 6가지 요건을 모두 충족할 때에 한해 무형자산(개발비)으로 자본화한다.",
      "내부창출 인식 제한: 내부적으로 창출한 영업권, 브랜드, 제호, 출판표제, 고객목록 등은 무형자산으로 인식하지 않는다.",
      "후속측정: 원가모형 또는 재평가모형을 적용한다. 재평가모형은 활성시장이 존재하여 공정가치를 신뢰성 있게 측정할 수 있는 경우에만 사용할 수 있다.",
      "내용연수: 유한 내용연수와 비한정 내용연수로 구분한다. 비한정이라는 것은 내용연수가 무한이라는 의미가 아니라 효익이 지속될 기간에 예측가능한 한도가 없다는 뜻이다.",
      "상각과 손상: 유한 내용연수 무형자산은 내용연수에 걸쳐 상각하며 잔존가치는 원칙적으로 영(0)으로 본다. 비한정 내용연수 무형자산과 아직 사용할 수 없는 무형자산은 상각하지 않고 매년 손상검사(제1036호)를 수행한다."
    ] },
    ...
    { "no": "1105", "ifrs": "IFRS 5", "title": "매각예정비유동자산과 중단영업", "category": "자산", "summary": "매각예정으로 분류되는 비유동자산(처분자산집단)의 측정·표시와 중단영업의 표시를 규정한다. 매각예정 분류요건, 장부금액과 순공정가치 중 작은 금액 측정, 감가상각 중단, 별도 표시, 중단영업의 세후 단일금액 표시를 다룬다.", "keywords": ["매각예정", "비유동자산", "처분자산집단", "중단영업", "순공정가치", "처분부대원가", "감가상각 중단", "세후 단일금액"], "chunks": [
      "매각예정 분류요건: 비유동자산(또는 처분자산집단)의 장부금액을 계속사용이 아니라 매각거래로 회수할 것으로 예상되면 매각예정으로 분류한다. 현재 상태로 즉시 매각가능하고, 매각이 매우 높은 가능성(적극적 매각계획·합리적 가격 제시, 보통 1년 내 완료 예상)이어야 한다.",
      "측정: 매각예정 비유동자산은 장부금액과 순공정가치(공정가치-처분부대원가) 중 작은 금액으로 측정한다. 매각예정으로 분류된 자산(또는 처분자산집단 내 자산)은 감가상각을 중단한다.",
      "손상과 환입: 순공정가치가 장부금액보다 낮으면 손상차손을 인식하고, 이후 순공정가치가 증가하면 과거 인식한 누적 손상차손을 한도로 이익을 인식한다.",
      "표시: 매각예정 비유동자산과 처분자산집단의 자산·부채는 재무상태표에서 다른 자산·부채와 구분해 별도로 표시한다.",
      "중단영업: 이미 처분되었거나 매각예정으로 분류된, 별도의 주요 사업계열이나 영업지역 등에 해당하는 부분을 중단영업으로 본다. 포괄손익계산서에 중단영업의 세후 손익을 단일금액으로 구분 표시한다."
    ] },
