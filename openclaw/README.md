...
### 번역 두뇌 선택: 로컬 Qwen vs GLM 5.2 클라우드

`translate.sh`는 대화용 두뇌와 별개로 번역에 쓸 LLM을 지정합니다. GLM 5.2(753B)는
48GB 맥미니에 **로컬로 안 들어가므로**, GLM을 쓰려면 Z.ai 클라우드 API로 붙습니다.
`.env.example`를 `.env`로 복사해 선택하세요 (`.env`는 `.gitignore`로 커밋 제외):

- **로컬 Qwen (무료·오프라인)**: 기본값. LM Studio에 Qwen3.6-35B-A3B 로드된 상태면 설정 불필요.
- **GLM 5.2 (클라우드·고품질)**: `.env`에 `LLM_URL`/`LLM_MODEL=glm-5.2`/`LLM_API_KEY` 지정.

번역·기사 요약 품질은 GLM 5.2가 더 좋지만, 로컬 Qwen은 무제한·무료·오프라인입니다.
